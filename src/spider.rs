    ...
    for id in ids {
        ...
        join_set.spawn(async move {
            let res = reqwest::Client::new()
                .get(format!("https://music.163.com/playlist?id={}", it))
                .send()
                .await?;
            let oneselect = scraper::Selector::parse("div.user > span.name > a").unwrap();
            if let Some(name) = scraper::Html::parse_document(&res.text().await?)
        .select(&oneselect)
        .next(){}
        });
    }
